...
def get_items_from_url_with_playwright(url, p, book_id):
    ...
    while loops:
        try:
            ...
            title = ''.join(c for c in title if c.isalpha() or c.isspace()).replace(' ','_')
                    ...
                    try:
                        ...
                        if video_element:
                            ...
                            if src:
                                ...
                                filename = f"{item_title}_ep_{ep}.mp4"
                                filepath = os.path.join(title_dir, filename)
                                response = requests.get(src)
                                with open(filepath, 'wb') as f:
                                    f.write(response.content)
                                print("Upload data")
                                s3_url = upload_to_s3(ep, book_id, filepath)
                                print("s3_url : ",s3_url)
                                if s3_url:
                        ...
                        pass
                br.close()
                shutil.rmtree(os.path.join(os.path.dirname(__file__), "browser_data_"+str(ep)))
...
def get_items_with_playwright(page, p, book_id):
    """
                        ...
                        try:
                            ...
                            if video_element:
                                ...
                                if src:
                                    ...
                                    item_title = title.lower().replace(' ','_')
                                    title_dir = os.path.join(content_dir, item_title.replace('/', '_').replace('\\', '_'))
                                    if not os.path.exists(title_dir):
                                        os.makedirs(title_dir, exist_ok=True)
                                        print(f"Created folder: {title_dir}")
...
def scrape_by_url(url, book_id):
    ...
    with sync_playwright() as p:
        ...
        url_input = url
        if url_input:
            get_items_from_url_with_playwright(url_input, p, book_id)
            ...
            playwright_items = get_items_with_playwright(page, p, book_id)
